...
for k = 1, nb_train do
	...
	state = {math.random(1, 10), math.random(1, 10)}

	newstate = table.copy(state) -- make a deep copy
	action = Brain.forward(newstate) -- returns index of chosen action

	if ((state[1] > state[2]) and (action == 1)) or ((state[1] == state[2]) and (action == 2)) or ((state[1] < state[2]) and (action == 3)) then
		reward = 1
	else
		reward = -2
	end
...
for k = 1, nb_test do
	...
	if ((state[1] > state[2]) and (output == 1)) or ((state[1] == state[2]) and (output == 2)) or ((state[1] < state[2]) and (output == 3)) then
		cnt = cnt + 1
	else
		print(state[1] .. " _ " .. state[2] .. " -> " .. output)
	end
end
